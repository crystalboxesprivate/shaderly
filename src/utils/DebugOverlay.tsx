...
class DebugOverlay extends React.Component<any, OverlayState> {
  ...
  constructor(props: any) {
    ...
    Events.addListener(
      'debugoverlaymessage',
      (msgInfo: { slot: number; msg: string }) => {
        this.setState({ update: !this.state.update })
  ...
  getMessageLog() {
    ...
    if (this.messages.length > 0) {
      ...
      this.messages.forEach((x, i) =>
        m.push(<Text key={`logi_${i}`}>{`${x}`}</Text>)
      )
      m.push(<Text key={`br`}>{'\n'}</Text>)
    }
    for (
      let x = this.log.length;
      x >= Math.max(this.log.length - (this.props.maxMessages || 10), 0);
      x--
    ) {
      if (this.log[x] == null) {
        continue
  ...
  render() {
    ...
    return (
      <View style={styles.overlayBlock}>
        <Text style={styles.text}>{this.getMessageLog()}</Text>
      </View>
    )
  }
}
...
const undoableOperationLog = (msg: string) =>
  Events.invoke('debugoverlaylog', `W: Undoable operation(${msg})`)
